...
$(function (){
    ...
    var id_str = '';
    $('.uploadFileImg').on('fileuploaded', function(event, data, previewId, index) {
        if(typeof(data.response.id) != 'undefined'){
            id_str = id_str+data.response.id+',';
        }
    });
    // filebatchuploadcomplete此事件仅在ajax上传和完成同步或异步ajax批量上传后触发。
    $('.uploadFileImg').on('filebatchuploadcomplete',function (event,files,extra) {
        if(id_str.length == 0){
